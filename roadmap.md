...
- [ ] **Regenerate the stale "Journal"-branded screenshot, then submit v1.0.3.** This is the nearest-term shippable task in the repo and it only existed in `CLAUDE.md`. v1.0.3 *metadata* went out 2026-08-02 (subtitle now "RSS & Atom feed reader", description/keywords rewritten), but `ios/.asc/screenshots/01-list-light.png` still shows the old single-feed "Journal" list UI. The app only auto-subscribes to journal.heyitsmejosh.com on a fresh install, so the screenshot reads as a dedicated journal app. Steps: add 1-2 demo feeds in the sim, recapture, `asc screenshots upload`, then submit v1.0.3 for review.
