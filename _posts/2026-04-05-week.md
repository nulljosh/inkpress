...
[Monica](https://github.com/nulljosh/monica) took up most of the weekend. I renamed it from Opticon and immediately broke the live site. Spent a full day reading through error logs before realizing the actual problem was hidden behind a crash page. Ten minute fix once I could actually see what was wrong.
...
[Spark](https://github.com/nulljosh/spark) got threaded comments, then I went all in on the Apple side. Rebuilt the iPhone app with profiles, badges, and sorting. Built a Mac app, a watch app, and widgets for all three. Fixed a voting bug where it was counting things twice and locked down the database. Mac, iPhone, watch, widgets -- same logic, four passes. The fourth pass is where you find all the places you made bad assumptions on the first.
...
Added five games to [Lingo](https://github.com/nulljosh/apps) -- chess, 2048, memory match, minesweeper, and snake.

Found a leaked API key in the Xcode build files and scrubbed it out.

Spent the evening consolidating again. Moved bots into the [systems](https://github.com/nulljosh/systems) monorepo, reorganized Dose, and cleaned up dead projects on Vercel. Down to 10 active repos. Also fixed the auto-commit watcher so it stops spamming empty lines into journal entries.
...
Changed the [Spark](https://github.com/nulljosh/spark) feed from a vertical list to a two-column grid on desktop, single column on mobile. Added six starter posts so the feed has content on first load.

Moved the [jot](https://github.com/nulljosh/systems) programming language into the systems monorepo. The old standalone repo is archived.
...
Got the [BC Garage Doors](https://github.com/nulljosh/bcgd) project properly set up. The landing page and the inventory dashboard were living in two separate places with no connection between them, so I merged them into one project and gave each its own subdomain.
...
Spent the day on [Monica](https://github.com/nulljosh/monica). Started with a blank map, 33 GitHub repos, and a version number that was lying.
...
Built a gamified RPG task manager called [Quest](https://github.com/nulljosh/apps) in one session. The concept: a fantasy quest journal for tracking real-life tasks, with XP, leveling (Squire through Mythic), difficulty ranks (F through S), dopamine rewards (80% chance on completion), and streak tracking.
...
Then the real move: merged everything into the [NYC colony sim](https://github.com/nulljosh/apps). Colonists now perform your real-life quests. Add "Go to the gym" as a fitness quest, and a colonist walks to the Gym building, shows a speech bubble saying "Lifting weights", works for 60 ticks, earns XP. Every 5 quests in a category boosts colonist stats matching that class (Warrior gets STR/END from fitness). Quest board built directly into the HUD (Q key). Player profile shows level, title, XP bar, streak.
