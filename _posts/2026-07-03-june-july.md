...
**In flight:** Epiphany v2.6.0 (live + App Store). Talli v3.5.1 (iOS/Mac Waiting for Review). Echo v1.3.3 (iOS resubmitted after rejection, IAP bundling risk pending verification). Healstack v2.3.3 (build valid, awaiting submit). Spark v2.1.0 (Mac live, iOS 4 screenshots remaining). Inkpress v1.0.2 (iOS build in progress to TestFlight). Lexly v1.1.0 (iOS/Mac submitted for review). Casewright v1.0.1 (iOS production-ready). Portfolio iOS app v1.0.0 (ASC id 6788180394, 9/10 blockers cleared, availability pending then ready to submit).

**Active development:** BCGD (web retheme live, iOS app scaffolded); Portfolio (iOS app submission imminent); Echo Mac build + upload pending; Era Context integration (onboarding done, capabilities unlocking). All test suites passing.
