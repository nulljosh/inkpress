...
Smoothed out the Markets news drawer drag that was choppy from fighting a declarative animation—removed the conflicting `.animation` modifier so the gesture now tracks the finger 1:1, and gave the drawer a semi-transparent liquid-glass look with `.ultraThinMaterial` and a hairline border rim.
