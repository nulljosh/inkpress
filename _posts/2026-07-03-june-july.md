...
**Shipped:** Talli v3.5.4 (iOS + Mac icon rebrand to blue #5B9BD5 + alpha channel fix on large icons + scraper junk filter hardening, build uploaded to TestFlight 2026-07-09, pending verification). Epiphany v2.6.1 (terminal TUI dashboard added for live portfolio polling). Echo iOS 1.3.3 (Waiting for Review, IAP bundling verified in resubmission). Inkpress iOS v1.0.2 (Waiting for Review, dark-mode CSS fix applied). Lexly v1.1.0 (iOS + Mac submitted, Stripe Pro checkout scaffolded). Casewright iOS 1.0.1 (Waiting for Review). Nimble landing page (GitHub Pages live). Portfolio site dark mode + perf live, iOS app submission imminent.

**Active:** Epiphany web live with Google OAuth + venue Look Around preview + Markets polish + landscape orientation re-enabled + Twitter OAuth wired (awaiting TWITTER_CLIENT_ID/SECRET). Healstack v2.3.4 (build valid, ready to submit). Spark v2.1.0 (Mac live, iOS blocked on App Group entitlement propagation). Books iOS + macOS builds ready for TestFlight. NYC macOS 1.0 + iOS 1.0.0 (5 blockers fixed, 1 validation error pending). BCGD web rethemed, iOS companion scaffolded.
