...
**Shipped:** Talli v3.5.4 (iOS + Mac icon rebrand to blue #5B9BD5 + alpha channel fix on large icons + scraper junk filter hardening, build uploaded to TestFlight 2026-07-09, pending verification). Echo iOS 1.3.3 (Waiting for Review, IAP bundling verified in resubmission). Inkpress iOS v1.0.2 (Waiting for Review, dark-mode CSS fix applied). Lexly v1.1.0 (iOS + Mac submitted). Casewright iOS 1.0.1 (Waiting for Review). Nimble landing page (GitHub Pages live). Portfolio site dark mode + perf live, iOS app submission imminent.
