...
Evening: completed Lexly's marketing landing page split. Added `/index.html` as a public-facing intro site with course highlights, unified header styling between the landing page and the `/app/` shell, and verified that native iOS/macOS SwiftUI already matches the web's accent color (#5B9BD5), closing out the visual parity roadmap item. App now lives at `/app/index.html` with the landing at the domain root.

**In flight:** Epiphany v2.6.0 (live + App Store). Talli v3.5.1 (iOS/Mac Waiting for Review). Echo v1.3.3 (iOS resubmitted after rejection, IAP bundling risk pending verification). Healstack v2.3.3 (build valid, awaiting submit, blocked by missing distribution profile). Spark v2.1.0 (Mac live, iOS blocked by missing distribution profile). Inkpress v1.0.2 (iOS build in progress to TestFlight). Lexly v1.1.0 (iOS/Mac submitted for review, landing page + app shell split shipped with unified header styling, native color parity confirmed). Casewright v1.0.1 (iOS production-ready). Portfolio iOS app v1.0.0 (ASC id 6788180394, 9/10 blockers cleared, availability pending then ready to submit, icon fix uncommitted).
