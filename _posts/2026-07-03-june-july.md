...
Talli got a xcstrings cleanup pass to remove drifted translations for zh and pa locales on the report.fileNow identifier, keeping the locale catalog clean before the next submission cycle.

Expanded Epiphany's asset universe with ten major bank stocks: RY, TD, BMO, BNS, CM (Canada), JPM, BAC, WFC, GS, MS (US). Added to DEFAULT_SYMBOLS in the server so they roll out across iOS, macOS, and watchOS. All 397 tests pass; pushed and auto-deployed.

**In flight:** Epiphany v2.6.0 (live + App Store). Talli v3.5.1 (iOS/Mac Waiting for Review). Echo v1.3.3 b8 (iOS Waiting for Review). Healstack v2.3.3 (build valid, awaiting submit). Spark v2.1.0 (Mac live, iOS 4 screenshots remaining). Inkpress v1.0.1 b3 (iOS resubmitted for review). Lexly v1.1.0 (iOS/Mac submitted for review). Casewright v1.0.1 (iOS production-ready). Portfolio iOS app v1.0.0 (ASC id 6788180394, 9/10 blockers cleared, availability pending then ready to submit).

**Active development:** BCGD (web retheme live, iOS app scaffolded); Portfolio (iOS app submission imminent); Echo Mac build + upload pending. All test suites passing.
