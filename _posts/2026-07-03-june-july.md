...
**Shipped:** Talli v3.5.4 (iOS + Mac icon rebrand to blue #5B9BD5 + alpha channel fix on large icons + scraper junk filter hardening, build uploaded to TestFlight 2026-07-09, pending verification). Epiphany v2.6.1 (terminal TUI dashboard added for live portfolio polling). Echo iOS 1.3.3 (Resubmitted after rejection fix — paywall now presents over Settings sheet; READY_FOR_REVIEW). Echo Mac 1.3.3 (build uploaded with corrected icon margins, proper 824px art centered on 1024px canvas). Inkpress iOS v1.0.2 (Waiting for Review, dark-mode CSS fix applied). Lexly v1.1.0 (iOS + Mac submitted, Stripe Pro checkout scaffolded). Casewright iOS 1.0.1 (icon badge removed and regenerated, Waiting for Review). Nimble landing page (GitHub Pages live). Portfolio site dark mode + perf live; Portfolio iOS app icon fixed to full-bleed; submission imminent. Spark v2.1.0 (Mac live, YC RFS Inspiration feature added to iOS + macOS; body font switched to sans). Books site typography updated to match portfolio (--font-body sans). Braingraph edge-spring fix + memory graph viewer shipped. Books library tracking updated (due date July 29).
...
**Active:** Epiphany web live with Google OAuth + venue Look Around preview + Markets polish + landscape orientation re-enabled + Twitter OAuth wired (awaiting TWITTER_CLIENT_ID/SECRET). Healstack v2.3.4 (build uploaded, icon live, ready to submit). Spark Mac live with YC RFS integration, iOS + all widgets blocked on manual App Group portal assignment (6 assignments remaining after Talli Mac completed). NYC macOS 1.0 + iOS 1.0.0 (5 blockers fixed, 1 validation error pending). BCGD web rethemed, iOS companion scaffolded.
