...
Lexly's fixed build (202607060001) uploaded to App Store Connect. Echo got its model-download watchdog hardened: added a 3-retry loop and capped iOS auto-model at the base version to avoid stalls. Updated the project portfolio (nulljosh.github.io) to point Nimble at its own domain (nimble.heyitsmejosh.com), added Healstack and Quotable to the projects table, and wired all the Pages deploy cancellation logic for cleaner CI runs.
...
**Shipped:** Epiphany v2.6.0 (iOS/macOS), Talli v3.5.1 (iOS/Mac in review), Casewright v1.0.1 (iOS Waiting for Review), Inkpress v1.0.1 b2 (iOS submitted for review).

**In flight:** Echo v1.3.3 b8 (iOS attached to ASC version record, was rejected 1.3.0; availability config + resubmit one command away). Lexly build 202607060001 live (iOS/Mac screenshots + metadata pending). Healstack v2.3.3 (build valid, awaiting submit). Spark v2.1.0 (Mac live, iOS 4 screenshots remaining). Echo Mac build + upload pending.

**Active development:** Epiphany (ticker universe expanded: DELL, PENG, DRAM, SNDK, IREN, MU, NBIS, CRDO, MRNA); Quotable, portfolio, and Healstack (deploy concurrency improvements); Talli (payment + messaging fixes, tests updated); Echo (model download watchdog + retry logic). All test suites passing.
