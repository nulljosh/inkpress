...
Late in the evening: folded quotable, wiretext, grapher, etyma, and braingraph into the labs monorepo via subtree merge (locally complete, push + archiving pending tomorrow). Vercel site repointing for grapher/wiretext/etyma also pending; live sites stay online throughout. Five standalone repos consolidated into one, streamlining maintenance while preserving full commit history. Newsline RSS clustering v0 shipped with bias-bar frontend (deploy blocked on wrangler auth).

Root (etyma) shipped as a Lookup-style v1: live Wiktionary lookup + etymology chain + word of the day across web + native iOS. iOS build succeeded and is ready for submission. NYC got a simplification pass: 3-pill toolbar, lean colonist panel, removed dead SAVE button + debug FPS overlay.

## Apps Summary

**Shipped:** Talli v3.5.4 (iOS + Mac icon rebrand to blue #5B9BD5 + alpha channel fix on large icons + scraper junk filter hardening, build uploaded to TestFlight 2026-07-09, pending verification; CGEB tracking added to Status tab with payment schedule). Epiphany v2.6.1 (terminal TUI dashboard added for live portfolio polling). Echo iOS 1.3.3 (Resubmitted after rejection fix — paywall now presents over Settings sheet; READY_FOR_REVIEW). Echo Mac 1.3.3 (build uploaded with corrected icon margins, proper 824px art centered on 1024px canvas). Inkpress iOS v1.0.2 (Waiting for Review, dark-mode CSS fix applied). Lexly v1.1.0 (iOS + Mac submitted, Stripe Pro checkout scaffolded). Casewright iOS 1.0.1 (icon badge removed and regenerated, Waiting for Review). Nimble (Gemma answer proxy deployed live to Cloudflare Worker, gemma-4-31b-it verified, header auth + thought filtering working). Spark v2.1.0 (Mac live, YC RFS Inspiration added to iOS + macOS, Gemma daily auto-ideas cron deployed with server-side enrichment fallback, body font unified to sans). Portfolio site dark mode + perf live; Portfolio iOS app icon fixed to full-bleed; submission imminent. Books site typography updated to match portfolio (--font-body sans). Braingraph (force-directed memory-graph web app with sage vibe and Inter typography shipped; roadmap: companion iOS/macOS app). Root/Etyma (Lookup-style v1: live Wiktionary lookup + etymology chain + word of the day, web + iOS shipped, iOS build succeeded). Books library tracking updated (due date July 29).

**In progress:** Books iOS build uploaded with icon (canvas scaling bug present). Portfolio iOS app submission (icon fixed). Echo iOS 1.3.3 (READY_FOR_REVIEW, build 9). TestFlight internal groups created for 6 apps (auto-distribution to phone now live).

**Active:** Epiphany web live with Google OAuth + venue Look Around preview + Markets polish + landscape orientation re-enabled + Twitter OAuth wired (awaiting TWITTER_CLIENT_ID/SECRET). Healstack v2.3.4 (build uploaded, icon live, ready to submit). Spark Mac live with YC RFS + Gemma ideas, iOS + all widgets blocked on manual App Group portal assignment (6 assignments remaining after Talli Mac completed). NYC macOS 1.0 + iOS 1.0.0 (5 blockers fixed, 1 validation error pending). BCGD web rethemed (teal brand), iOS companion app verified in simulator (all 4 tabs tested end-to-end), screenshots added to README, 1024px icon asset catalog ready—ASC ship pending bundle ID registration + app record creation.
