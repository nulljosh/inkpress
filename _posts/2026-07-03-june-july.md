...
Talli's icon rebrand wrapped up tonight: finished the orange-to-blue color shift by recoloring the tally-mark icon source to #5B9BD5 on a #1a1a1a background, then regenerated all iOS AppIcon variants (light, dark, and tinted) plus LaunchIcon PNGs, fixing the brown/orange TestFlight app icon and the launch/login screens. Also hardened the scraper by moving the JUNK_RX accessibility filter into the shared uniqueTrimmed function, stopping skip-nav boilerplate ("Skip to main content", "Accessibility Statement") from leaking into the iOS Status Messages and Messages tab. Bumped Talli docs to v3.5.4, with all changes committed and pushed on branch fix/blue-rebrand-icon-scraper.
...
**Shipped:** Talli v3.5.4 (iOS + Mac icon rebrand to blue #5B9BD5 + scraper junk filter hardening on fix/blue-rebrand-icon-scraper branch, docs bumped, ready for TestFlight icon fix build upload). Echo iOS 1.3.3 (Waiting for Review, IAP bundling verified in resubmission). Inkpress iOS v1.0.2 (Waiting for Review, dark-mode CSS fix applied). Lexly v1.1.0 (iOS + Mac submitted). Casewright iOS 1.0.1 (Waiting for Review). Nimble landing page (GitHub Pages live). Portfolio site dark mode + perf live, iOS app submission imminent.

**Active:** Epiphany iOS 2.5.2 (Waiting for Review), web live with Google OAuth + venue Look Around preview + Markets polish. Healstack v2.3.4 (build valid, ready to submit). Spark v2.1.0 (Mac live, iOS blocked on App Group entitlement propagation). Books iOS + macOS builds ready for TestFlight. NYC macOS 1.0 + iOS 1.0.0 (5 blockers fixed, 1 validation error pending). BCGD web rethemed, iOS companion scaffolded.

**Blockers:** Spark iOS App Group entitlement (UIRegistration shows group checked but profiles regenerate empty). Epiphany screenshot edits (ASC locked during review). NYC last validation error. Books distribution profiles (waiting on asc-signing-setup). Portfolio iOS submission (availability pending).
