...
Epiphany got two auth commits pushed: reverted the landscape orientation flag (needs a full per-screen layout pass before that's safe), and shipped Sign in with X (Twitter) OAuth to complement GitHub + Apple OAuth on the web dashboard. Scaffolded a Claude Design System project for the portfolio (heyitsmejosh.com) documenting component tokens: color palette, typography scales, link states, list sections, and header/bio card — created 5 example component cards authored by hand from the tokens-sage.css values since the URL importer rejected the page as too simple.

## Apps Summary

**Shipped:** Talli v3.5.3 (iOS + Mac Waiting for Review, incorrect v2.4.4 submission cancelled and corrected). Echo iOS 1.3.3 (Waiting for Review, IAP bundling verified in resubmission). Inkpress iOS v1.0.2 (Waiting for Review, dark-mode CSS fix applied). Lexly v1.1.0 (iOS + Mac submitted). Casewright iOS 1.0.1 (Waiting for Review). Nimble landing page (GitHub Pages live). Portfolio site dark mode + perf live, iOS app submission imminent.
